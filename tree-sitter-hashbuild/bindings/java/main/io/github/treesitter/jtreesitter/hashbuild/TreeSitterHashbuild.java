package io.github.treesitter.jtreesitter.hashbuild;

import java.lang.foreign.*;

public final class TreeSitterHashbuild {
    ...
    private static final TreeSitterHashbuild INSTANCE = new TreeSitterHashbuild();

    private final Arena arena = Arena.ofAuto();
    private volatile SymbolLookup lookup = null;

    private TreeSitterHashbuild() {}
    ...
    public static MemorySegment language(SymbolLookup lookup) {
        return call(lookup, "tree_sitter_hashbuild");
    }

    private SymbolLookup findLibrary() {
        try {
            var library = System.mapLibraryName("tree-sitter-hashbuild");
            return SymbolLookup.libraryLookup(library, arena);
        } catch (IllegalArgumentException ex1) {
            try {
                System.loadLibrary("tree-sitter-hashbuild");
